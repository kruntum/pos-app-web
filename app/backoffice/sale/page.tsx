...
export default function SalePage() {
    ...
    const unSelectTaste = async (tasteId: number, saleTempDetailId: number, saleTempId: number) => {
        try {
            const payload = {
                saleTempDetailId: saleTempDetailId,
            }
...
console.log(res.data.fileName);
    ...
    const openModalEdit = (item: any) => {
        //     fetchDataTaste(item.Food.foodTypeId);
        ...
        <>
            <div className="card mt-3">
                ...
                <div className="card-body">
                    ...
                    <div className="row mt-3">
                        ...
                        <div className="col-md-3">
                            <div className="alert py-4 p-2 text-end text-4xl text-white bg-dark">
                                {(amount + amountAdded).toLocaleString('th-TH', { style: 'currency', currency: 'THB' })}
                            </div>
                            {amount > 0 ?
                                <button
            ...
            <MyModal id="modalEdit" title="แก้ไขรายการ" modalSize="modal-xl">
                ...
                <table className="table table-bordered mt-3">
                    <thead>
                        <tr>
                            <th className="text-center w-[70px]">#</th>
                            <th className="text-center w-[150px]">ชื่ออาหาร</th>
                            <th className="text-center w-[150px]">รสชาติ</th>
                            <th className="text-center w-[150px]">ขนาด</th>
                        </tr>
                    </thead>
                    <tbody>
                        {saleTempDetails.map((item: any) =>
                            <tr key={item.id}>
                                ...
                                <td className="flex justify-start items-center h-14 ">
                                    <p className="m-2">{item.Food.name}</p>
                                </td>
                                <td className="text-center">
                                    {tastes.map((taste: any) =>
                                        item.tasteId === taste.id ?
                                            <button
                                                onClick={e => unSelectTaste(taste.id, item.id, item.saleTempId)}
                                        ...
                                        size.moneyAdded > 0 ?
                                            item.foodSizeId === size.id ?
                                                ...
                                                <button
                                                    ...
                                                    className="btn btn-outline-success me-1" key={size.id}>+{size.moneyAdded} {size.name}</button>
                                            :
                                            <></>
                                    )}
                                </td>
            ...
            <MyModal id="modalPrint" title="พิมพ์เอกสาร">
                {billUrl && <iframe src={config.apiServer + '/' + billUrl} width="100%" height="800px"></iframe>}
            </MyModal>
        </>
    )
}